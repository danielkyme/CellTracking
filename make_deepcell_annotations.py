...
# Convert annotations from Crowdflower into a format that can be fed into Deepcell
...
def generate_tripartite_annotations( images_folders ):
    for image_folder in images_folders:
        ...
        annotated_images = [ f for f in os.listdir( input_folder ) 
        ...
        for annotated_image in annotated_images:
            ...
            output_interior_filepath = os.path.join( output_interior_folder, annotated_image + "_new" )
            cv2.imwrite( output_interior_filepath, output_interior_array )
            output_border_filepath = os.path.join( output_border_folder, annotated_image  + "_new" )
            cv2.imwrite( output_border_filepath, output_border_array )
            output_background_filepath = os.path.join( output_background_folder, annotated_image + "_new" )
